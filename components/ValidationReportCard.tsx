...
export const ValidationReportCard: React.FC<ValidationReportCardProps> = ({ report }) => {
    ...
    return (
        <CollapsibleCard
            title={title}
            icon={<status.Icon className={`w-6 h-6 ${status.classes}`} />}
            defaultOpen={report.issues.some(i => i.severity === 'critical' || i.severity === 'warning')}
        >
            <div className="p-4 bg-surface">
               ...
               {report.issues.length > 0 && (
                   <div className="mt-6">
                       <h4 className="font-bold text-text-primary mb-2 text-left">Issues Found:</h4>
